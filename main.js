...
const LAST_ORDER_KEY = "eg_last_order"; // ✅ invoice/tracking
...
/* ⚠ cat dropdown is not in your HTML now, keep optional */
...
function validateCoupon(code, subtotal){
  ...
  if(c === "EG10" && subtotal >= 5000) return { code:c, type:"percent", value:0.5 };
...
function calcDiscount(coupon, subtotal, delivery){
  ...
  if(coupon.type === "ship") return delivery; // free shipping value
  return 0;
}

/* ===== INVOICE/TRACKING STORAGE ===== */
function getLastOrder(){ return JSON.parse(localStorage.getItem(LAST_ORDER_KEY) || "null"); }
function setLastOrder(o){ localStorage.setItem(LAST_ORDER_KEY, JSON.stringify(o)); }
...
function updateCompare(){
  ...
  compareItems.innerHTML = compare.map((c,i)=>`
    <div class="compare-row">
      <div class="compare-name" title="${c.name}">${c.name}</div>
      <div class="compare-price">${(Number(c.price)===0) ? "Free" : `৳${(Number(c.price)||0).toLocaleString("en-US")}`}</div>
...
function applyGames(){
  ...
  const sub = $("#gamesSub");
  if(sub){
    sub.textContent = (genre === "all")
      ? "Choose your favorite games. Instant digital delivery."
      : `Showing genre: ${genre}`;
  }
}
  ...
  if(cartItems){
    if(!cart.length){
      ...
      cartItems.innerHTML = cart.map((c, i)=>{
        ...
        const priceText = (Number(c.price) === 0) ? "Free" : `৳${(Number(c.price)||0).toLocaleString("en-US")}`;
        return `
          <div class="cart-row">
...
function calcCheckoutTotals(cart){
  ...
  // ship coupon (freeship) reduces delivery, not subtotal
...
function renderCheckout(){
  ...
  summaryList.innerHTML = cart.map((c, i)=>{
...
function buildTrackingHTML(order){
  ...
  const li = steps.map((t, i)=>{
    const n = i+1;
    const cls = n < stage ? "done" : (n === stage ? "active" : "");
    const desc = n === 1 ? "We received your order." :
                 n === 2 ? "Preparing items / verifying payment." :
                 n === 3 ? (hasGames ? "Sending codes to email." : "Rider is on the way.") :
                 "Order completed.";
    return `
      <li class="${cls}">
...
function attachEvents(){
  ...
  /* ✅ COUPON APPLY (safe even if DOM missing) */
  applyCouponBtn?.addEventListener("click", ()=>{
    if(!requireLogin()) return;

    const cart = getCart();
    if(!cart.length){
      if(couponMsg){ couponMsg.style.color="#ef4444"; couponMsg.textContent="❌ Cart is empty."; }
      return;
    }
    ...
    if(!valid){
      ...
      if(couponMsg){
        ...
        couponMsg.textContent="❌ Invalid coupon (Try: EG10 / EG200 / FREESHIP)";
      }
      renderCheckout();
      return;
    }
  ...
  cartItems?.addEventListener("click", (e)=>{
    ...
    setCart(cart);
    refreshCartUI();
    renderCheckout(); // keep totals updated if on checkout
  });
  ...
  $("#contactForm")?.addEventListener("submit", (e)=>{
    ...
    if(!name || !email || !msg){
      if(out){ out.style.color = "#ef4444"; out.textContent = "❌ Please fill all fields."; }
      return;
    }
    if(out){ out.style.color = "#16a34a"; out.textContent = "✅ Message sent! We will contact you soon."; }
    e.target.reset();
  });
  ...
  $("#registerForm")?.addEventListener("submit", (e)=>{
    ...
    if(!name || !email || !pass){
      if(msg){ msg.style.color = "#ef4444"; msg.textContent = "❌ Please fill all fields."; }
      return;
    }
    if(users.some(u => u.email === email)){
      if(msg){ msg.style.color = "#ef4444"; msg.textContent = "❌ This email is already registered."; }
      return;
    }
    ...
    if(msg){ msg.style.color = "#16a34a"; msg.textContent = "✅ Registration successful! Redirecting to login..."; }
  ...
  $("#loginForm")?.addEventListener("submit", (e)=>{
    ...
    if(!user){
      if(msg){ msg.style.color = "#ef4444"; msg.textContent = "❌ Invalid email or password."; }
      return;
    }
    ...
    if(msg){ msg.style.color = "#16a34a"; msg.textContent = "✅ Login successful! Redirecting..."; }
  ...
  placeOrderBtn?.addEventListener("click", ()=>{
    ...
    const cart = getCart();
    if(!cart.length){
      if(orderMsg){ orderMsg.style.color="#ef4444"; orderMsg.textContent="❌ Cart is empty."; }
      return;
    }
    ...
    if(ref.length < 3){
      if(orderMsg){ orderMsg.style.color="#ef4444"; orderMsg.textContent="❌ Please enter Transaction/Reference info."; }
      return;
    }

    const hasGames = cart.some(x => x.type === "game");
    if(hasGames && !email){
      if(orderMsg){ orderMsg.style.color="#ef4444"; orderMsg.textContent="❌ Please provide email for digital delivery."; }
      return;
    }

    const payName = pay === "bkash" ? "bKash" : pay === "nagad" ? "Nagad" : pay === "rocket" ? "Rocket" : "Card";

    // ✅ create order BEFORE clearing cart (for invoice)
    ...
    if(orderMsg){ orderMsg.style.color="#16a34a"; orderMsg.textContent="✅ Order placed successfully!"; }
